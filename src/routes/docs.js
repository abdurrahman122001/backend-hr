...
function writeNda(doc, emp) {
  ...
  // Document info: Date and Document Number on same line
  const dateDocNoText = `Date: ${dateString}                                                                 Doc No: ${docNumber}`;
  doc.font("Helvetica").fontSize(10).text(dateDocNoText, doc.page.margins.left, doc.y, {
  ...
  // Write all text segments with proper spacing
  ndaText.forEach(segment => {
    ...
    doc.moveDown(1); // Add one line gap after each point
  });
  ...
  doc.moveDown(3); // Add 2-line gap here
...
async function writeSalaryCertificate(doc, emp, issueDate, joinDate, monthlySalary) {
  ...
  // Document info: Date and Document Number on same line
  const docNumber = `SAL-${new Date().getTime().toString(36).toUpperCase()}`;
  const dateDocNoText = `Date: ${issueDate}                                                                 Doc No: ${docNumber}`;
  doc.font("Helvetica").fontSize(10).text(dateDocNoText, doc.page.margins.left, doc.y, {
    width: doc.page.width - doc.page.margins.left - doc.page.margins.right,
    align: "left"
  });
