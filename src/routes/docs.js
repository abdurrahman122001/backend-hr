...
// ========== FULL NDA AND CONTRACT CONTENTS ==========
...
Basic Compensation: Rs. ${gross.toLocaleString()}
Conveyance Allowance: Rs. ${conveyance.toLocaleString()}

${emp.name}, bearing CNIC number ${emp.cnic}

After your probation period of ${probation} months your performance will be evaluated on the basis of your monthly targets and Key Performance Indicators and the continuity of your employment with us dependent on those evaluations.
...
// Salary Certificate
function writeSalaryCertificate(doc, emp, issueDate, joinDate, monthlySalary) {
  // Fallbacks for missing fields
  const department = emp.department || "-";
  const designation = emp.designation || emp.position || "Employee";
  const cnic = emp.cnic || emp.nationalId || "-";
  const nationality = emp.nationality || "Pakistani";
  const hrManager = emp.hrManager || "ZARYAB UL HASSAN KHAN";
  const hrManagerDesignation = emp.hrManagerDesignation || "HR MANAGER";

  doc.fontSize(12)
    .font("Times-Roman")
    .text(issueDate, { align: "left" });
  ...
  doc.text(
    "Please feel free to contact the Human Resource Department for any further details, doubts and notifications.",
    { align: "left" }
  );
  doc.moveDown(1);
  ...
  doc.font("Times-Bold")
    .text(`${hrManager.toUpperCase()}`, { align: "left" });
  doc.font("Times-Roman")
    .text(`${hrManagerDesignation.toUpperCase()}`, { align: "left" });
}

// ===== PDF GENERATION HELPERS (updated to support dynamic join date and salary slip) =====
async function generateSalaryCertificatePdf(employee) {
  // Find latest salary slip if available (optional, for dynamic monthly salary)
  const slip = await SalarySlip.findOne({ employee: employee._id }).sort({ updatedAt: -1 }).lean();
  // Use grossSalary, fallback to employee compensation
  const monthlySalary = slip?.grossSalary || employee.compensation?.grossSalary || null;
  ...
  const doc = new PDFDocument({ margin: 70 }); // More margin for better print
  doc.pipe(fs.createWriteStream(pdfPath));
  writeSalaryCertificate(doc, employee, issueDate, joinDate, monthlySalary);
  doc.end();
  await waitForFileWrite(pdfPath);
  return pdfPath;
}
// ===== PDF GENERATION HELPERS (now use the full content above) =====
...
async function generateContractPdf(employee) {
  const slip = await SalarySlip.findOne({ employee: employee._id }).sort({ updatedAt: -1 }).lean();
  const pdfPath = path.join(UPLOAD_DIR, `contract_${employee._id}.pdf`);
  const doc = new PDFDocument({ margin: 50 });
  doc.pipe(fs.createWriteStream(pdfPath));
  writeContract(doc, employee, slip);
  doc.end();
  await waitForFileWrite(pdfPath);
  return pdfPath;
}
...
async function generateSalaryCertificatePdf(employee) {
  ...
  const doc = new PDFDocument({ margin: 50 });
  doc.pipe(fs.createWriteStream(pdfPath));
  writeSalaryCertificate(doc, employee);
  doc.end();
  await waitForFileWrite(pdfPath);
  return pdfPath;
}
...
router.get("/salary-certificate/:employeeId", async (req, res) => {
  ...
  if (!salaryCertPath || !fs.existsSync(salaryCertPath)) {
    salaryCertPath = await generateSalaryCertificatePdf(emp);
