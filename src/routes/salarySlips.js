...
// ---------- GET: Fiscal Year Salary Slips for Tax Calculation ----------
router.get("/fiscal-year/:employeeId", requireAuth, async (req, res) => {
  try {
    const { employeeId } = req.params;
    const { fiscalYearStart, fiscalYearEnd, encryptionKey } = req.query;
    ...
    if (currentUserRole === "super-admin") {
      ...
      const emp = await Employee.findOne({ _id: employeeId, owner: req.user.owner });
      if (emp) ownerAllowed = true;
    } else {
      // Regular employee can only see their own slips
    ...
    if (!ownerAllowed) {
      return res.status(403).json({
        status: "error",
        message: "Not authorized to view salary slips for this employee"
      });
    }

    // Determine fiscal year range
    // Default: July-June (Pakistan fiscal year)
    const now = new Date();
    const defaultFiscalStartMonth = 7; // July
    const defaultFiscalEndMonth = 6;   // June
    ...
    if (fiscalYearStart && fiscalYearEnd) {
      ...
      fiscalStartYear = startDate.getFullYear();
      fiscalEndYear = endDate.getFullYear();
    } else {
      // Calculate based on current date
      const currentMonth = now.getMonth() + 1; // 1-indexed
      if (currentMonth <= defaultFiscalEndMonth) {
        // Jan-Jun: fiscal year started last year
        fiscalStartYear = now.getFullYear() - 1;
        fiscalEndYear = now.getFullYear();
      } else {
        // July-Dec: fiscal year ends next year
        fiscalStartYear = now.getFullYear();
        fiscalEndYear = now.getFullYear() + 1;
      }
    }

    // Build month-year combinations to query
    ...
    for (let m = 6; m < 12; m++) {
      fiscalMonths.push({ month: monthNames[m], year: String(fiscalStartYear) });
    }
    // January to June of end year
    for (let m = 0; m < 6; m++) {
      fiscalMonths.push({ month: monthNames[m], year: String(fiscalEndYear) });
    }

    // Fetch all slips for this employee in the fiscal year
    const allSlips = [];
    ...
    for (const { month, year } of fiscalMonths) {
      ...
      if (slips.length > 0) {
        ...
        // Decrypt key fields for tax calculation
        ...
        const fieldsToDecrypt = [
          ...
          "autoAllowance", "incentive", "fuelAllowance", "othersAllowances",
          "grossSalary", "leaveDeductions", "lateDeductions", "eobiDeduction",
          "sessiDeduction", "providentFundDeduction", "gratuityFundDeduction",
          "advanceSalaryDeductions", "medicalInsurance", "lifeInsurance",
          "penalties", "othersDeductions", "taxDeduction", "totalAllowances",
        ...
        for (const field of fieldsToDecrypt) {
          if (slipObj[field]) {
            try {
              const decrypted = await decrypt(slipObj[field], encKey);
              decryptedFields[field] = Number(decrypted) || 0;
            } catch {
              decryptedFields[field] = 0;
            }
          } else {
            decryptedFields[field] = 0;
          }
        ...
        const netSalary = decryptedFields.netPayable ||
          ...
          calcNet({ ...decryptedFields, ...slipObj });

        allSlips.push({
          _id: slipObj._id,
          month: slipObj.month,
          year: slipObj.year,
          isLocked: slipObj.isLocked,
          createdAt: slipObj.createdAt,
          decrypted: {
            ...decryptedFields,
            netSalary
          }
    ...
    const ytdGross = allSlips.reduce((sum, s) => sum + (s.decrypted.grossSalary || 0), 0);
    const ytdNet = allSlips.reduce((sum, s) => sum + (s.decrypted.netSalary || 0), 0);
    ...
    res.json({
      ...
      slips: allSlips,
      summary: {
        monthsCount: allSlips.length,
        ytdGross,
        ytdNet,
        ytdTax,
        ytdMedical,
        averageMonthlyNet: allSlips.length > 0 ? Math.round(ytdNet / allSlips.length) : 0
      }
    });
  } catch (err) {
    console.error("Error fetching fiscal year slips:", err);
    res.status(500).json({ status: "error", message: err.message });
  }
});
