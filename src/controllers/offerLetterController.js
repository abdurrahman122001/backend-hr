...
const SALARY_COMPONENTS = [
  "basic","dearnessAllowance","houseRentAllowance","conveyanceAllowance","medicalAllowance","utilityAllowance",
  "overtimeComp","dislocationAllowance","leaveEncashment","bonus","arrears","autoAllowance","incentive",
...
async function getCompanyContext(ownerId) {
  ...
  const address = (companyDoc?.address && String(companyDoc.address).trim()) || FALLBACKS.address;
...
function enforceComicSans(html) {
  ...
  const pRequired = [
    "margin:0 !important","margin-block-start:0","margin-block-end:0",
    "mso-margin-top-alt:0","mso-margin-bottom-alt:0","mso-line-height-rule:exactly",family,
  ].join("; ");
...
/* --------------------------- Helper: Formatting --------------------------- */
function formatDateDMY(dateInput){ if(!dateInput) return ""; const d=new Date(dateInput); if(isNaN(d.getTime())) return ""; return `${d.getDate()} ${d.toLocaleString("default",{month:"long"})} ${d.getFullYear()}`; }
function formatTime12hr(timeStr){ if(!timeStr) return ""; let [h,m] = timeStr.includes(":")?[+timeStr.split(":")[0],timeStr.split(":")[1]]:[+timeStr,"00"]; let suf="AM"; if(h>=12){suf="PM"; if(h>12) h-=12;} if(h===0) h=12; return `${h}:${String(m).padStart(2,"0")} ${suf}`; }
function normalizeTime(s){ if(!s||typeof s!=="string") return ""; const [h="00",m="00"]=s.split(":"); return `${String(h).padStart(2,"0")}:${String(m).padStart(2,"0")}`; }
function formatNumberWithCommas(x){ return Number(x).toLocaleString("en-PK"); }
function probationDaysToMonths(probationDays){ const days=Number(probationDays)||0; if(!days) return ""; const months=Math.round(days/30); return months>0?`${months} month${months>1?"s":""}`:`${days} days`; }
...
async function sendOfferLetter(req, res) {
  try {
    const {
      candidateEmail, candidateName, position,
      salaryBreakup = {}, startDate, reportingTime,
      confirmationDeadlineDate, department, shift, probationDays,
      subject: subjectOverride, letter: letterOverride, // optional from client editor
    } = req.body;

    if (!candidateEmail || !candidateName || !position || !startDate || !reportingTime || !confirmationDeadlineDate || !probationDays) {
      return res.status(400).json({ error: "Missing required fields for sending offer." });
    }
    if (!req.user || !req.user._id) {
      return res.status(401).json({ error: "No user context found." });
    }
    let ownerId = req.user._id;
    if (!(ownerId instanceof mongoose.Types.ObjectId)) ownerId = new mongoose.Types.ObjectId(ownerId);

    const exists = await Employee.findOne({ email: candidateEmail });
    if (exists) return res.status(400).json({ error: "An employee with this email already exists. Offer not sent." });

    // Build context and load template
    const { ctx, companyCtx, signatureBlock, safeCandidateName, grossSalaryRaw } =
      await buildContext({ ownerId, candidateName, candidateEmail, position, salaryBreakup, startDate, reportingTime, confirmationDeadlineDate, department, shift, probationDays });
    ...
    let finalSubject = subjectOverride || (tpl ? renderWithContext(tpl.subject || "", ctx) : `Offer of Employment – ${position} at ${companyCtx.name}`);
    let finalHtml    = letterOverride   || (tpl ? renderWithContext(tpl.html || "", ctx)    : `
    ...
    let employee = await Employee.create({
      name: sanitizeName(candidateName), email: candidateEmail, designation: position,
      joiningDate: startDate, department: department || null,
      owner: ownerId, createdBy: ownerId, rt: normalizedRT,
      shifts: shift ? [shift] : undefined,
      ...(probationDaysNum > 0 ? { leaveEntitlement: { total: 0, usedPaid: 0, usedUnpaid: 0 } } : {}),
    });
    if (probationDaysNum > 0) {
      await Employee.updateOne({ _id: employee._id }, { $set: { "leaveEntitlement.total": 0 } }, { runValidators: false });
      employee = await Employee.findById(employee._id);
    }

    const encryptedSalaryFields = await Promise.all(
      SALARY_COMPONENTS.map(async (k) => ({ [k]: await encrypt((salaryBreakup?.[k] || 0).toString()) }))
    );
    ...
    await Salaries.create({
      ...
      owner: ownerId, createdBy: ownerId,
