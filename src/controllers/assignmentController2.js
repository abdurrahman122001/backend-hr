...
exports.createMessage = async function createMessage(req, res) {
  try {
    const {
      ...
      cc: ccBody,
      // New field: autoSendToClient (for managers/team leads to send directly to client)
    ...
    let threadMessages = [];
    let emailThreadInfo = null;

    // If we have a replyTo, get the original message for thread info
    if (replyTo) {
    ...
    let shouldSendToClient = autoSendToClient; // Track if we should send to client
    ...
    if (client && isObjId(client)) {
      ...
      if (inheritedIsFromClient || inheritedIsFromCompanyEmployee) {
        ...
        approvalStatus = "approved";
        // For direct supervision, send directly to client if sender is manager or team lead
    ...
    // Get email thread info if client exists
    if (client && isObjId(client) && !emailThreadInfo) {
      emailThreadInfo = await getEmailThreadInfo(null, client);
    }
    ...
    const populated = await msg.populate([
      ...
      { path: "sender", select: "_id name companyEmail role" },
      { path: "receiver", select: "_id name companyEmail role" },
      { path: "client", select: "_id clientName clientEmail companyEmployees assignedTo" },
      { path: "scheduledBy", select: "_id name companyEmail" },
      { path: "attachments", select: "filename originalName mimetype size url" },
      { path: "replyTo", select: "_id subject emailMetadata" },
    ]);

    // 🔥 NEW LOGIC: If message should be sent directly to client (for managers/team leads with direct supervision)
    let emailSentToClient = false;
    let clientEmails = [];
    let emailError = null;
    let emailInfo = null;

    if (shouldSendToClient && populated.client && !isScheduled) {
      try {
        // Get client email addresses
        if (populated.client.clientEmail) {
          clientEmails.push(populated.client.clientEmail);
        }

        // Get company employee emails if any
        if (populated.client.companyEmployees && populated.client.companyEmployees.length > 0) {
          populated.client.companyEmployees.forEach(emp => {
            if (emp.email) {
              clientEmails.push(emp.email);
            }
          });
        }

        // Remove duplicates
        clientEmails = [...new Set(clientEmails.filter(email => email && email.includes('@')))];
        ...
        if (clientEmails.length > 0) {
          ...
          } else if (emailThreadInfo) {
            // Use the thread info
            inReplyTo = emailThreadInfo.messageId;
            references = emailThreadInfo.references;
          }
    ...
    res.status(201).json({
      ...populated.toObject(),
      emailInfo: {
        sentToClient: emailSentToClient,
        clientEmails: clientEmails,
        threadInfo: emailInfo ? {
          messageId: emailInfo.messageId,
          inReplyTo: emailInfo.inReplyTo,
          references: emailInfo.references
        } : null,
        error: emailError
      },
      isReply: !!populated.replyTo
...
async function getEmailThreadInfo(messageId, clientId) {
  try {
    // Find the original email that started this thread
    const originalMessage = await AssignmentMessage.findOne({
      $or: [
        { _id: messageId },
        { replyTo: messageId },
        { "emailMetadata.inReplyTo": messageId },
        { threadId: messageId }
      ],
      client: clientId,
    ...
    // If no email metadata found, generate thread info
    return {
      messageId: `<${Date.now()}.${Math.random().toString(36).substr(2)}@${process.env.MAIL_DOMAIN || 'virsme.com'}>`,
      threadId: `email_thread_${clientId}_${Date.now()}`,
      references: null
    };
  } catch (error) {
...
async function sendEmailToClient(emailDetails) {
  try {
    ...
    const mailOptions = {
      ...
      html: html || text || "",
      cc: cc || [],
      replyTo: process.env.SMTP_REPLY_TO || process.env.MAIL_USERNAME,
    ...
    if (attachments && attachments.length > 0) {
      mailOptions.attachments = attachments.map(att => {
        // Handle both URL and base64 attachments
        if (att.url && att.url.startsWith('data:')) {
          ...
          if (matches) {
            return {
              ...
              contentType: matches[1] || att.mimetype || 'application/octet-stream'
            };
          }
        } else if (att.url) {
          // Regular URL - would need to fetch the file
          return {
            filename: att.originalName || att.filename,
            path: att.url,
...
/** ---------- UPDATED APPROVE MESSAGE FUNCTION ---------- **/
exports.approveMessage = async function approveMessage(req, res) {
  try {
    ...
    const msg = await AssignmentMessage.findById(id).populate([
      ...
      {
        path: "client",
        select: "_id clientName clientEmail companyEmployees assignedTo",
      },
      { path: "owner", select: "_id name companyEmail" },
    ]);
    ...
    // Get managers for the owner (for internal notification)
    ...
    // 🔥 NEW LOGIC: Send message to client automatically
    let clientEmails = [];
    let emailSent = false;
    let emailError = null;
    ...
    if (populated.client) {
      try {
        ...
        // Get company employee emails if any
        if (
          populated.client.companyEmployees &&
          populated.client.companyEmployees.length > 0
        ) {
          populated.client.companyEmployees.forEach((emp) => {
            if (emp.email) {
              clientEmails.push(emp.email);
        ...
        clientEmails = [
          ...new Set(
            clientEmails.filter((email) => email && email.includes("@"))
          ),
        ];
        ...
        if (clientEmails.length > 0) {
          ...
          const emailContent = {
            to: clientEmails,
            subject: populated.subject || "Message from your service provider",
            text: populated.note || "",
            html: populated.note || "", // Use same content for HTML (plain)
            threadId: populated.threadId,
            replyTo: process.env.SMTP_REPLY_TO || process.env.MAIL_USERNAME,
            attachments:
              populated.attachments && populated.attachments.length > 0
          ...
          await sendEmailToClient(emailContent);
          emailSent = true;

          // Update message with email metadata
          msg.emailMetadata = {
            sentToClient: true,
            clientEmails: clientEmails,
            sentAt: new Date(),
    ...
    const io = getIO(req);
    if (io) {
      // Notify managers about approval (internal notification)
      if (managers.length > 0) {
      ...
      const currentReceivers = Array.isArray(populated.receiver)
        ? populated.receiver.map((r) => String(r))
        : [];
      ...
      // Emit general update for compatibility
      allParticipants.forEach((participantId) => {
        io.to(`employee_${participantId}`).emit("assignment_message_updated", {
          message: populated,
          action: "approved",
